...
import com.cafe.proejct.POJO.Product;
import com.cafe.proejct.constents.CafeConstants;
import com.cafe.proejct.dao.ProductDao;
import com.cafe.proejct.service.ProductService;
import com.cafe.proejct.utils.cafeutils;
import com.cafe.proejct.wrapper.ProductWrapper;
		...
		try {
			if(jwtFilter.isAdmin()) {
			  ...
			  if(!optional.isEmpty()) {
				  ...
				  return cafeutils.getResponseEntity("Product Status Updated Sucessfully", HttpStatus.OK);
			  }
				return cafeutils.getResponseEntity("Product id does not exist",HttpStatus.OK );
			}else {
				return cafeutils.getResponseEntity(CafeConstants.UNAUTHORIZED_ACCESS, HttpStatus.UNAUTHORIZED);
